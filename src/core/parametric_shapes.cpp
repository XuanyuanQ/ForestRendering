...
//	}
	...
	std::vector<glm::vec3> vertices = std::vector<glm::vec3>((horizontal_split_count + 1) * (vertical_split_count + 1));
	std::vector<glm::uvec3> index_sets = std::vector<glm::uvec3>(horizontal_split_count * vertical_split_count * 2);
	std::vector<glm::vec2> texcoords = std::vector<glm::vec2>((horizontal_split_count + 1) * (vertical_split_count + 1));
	...
	for(unsigned i = 0u;i < vertical_split_count + 1u; i++){
		for(unsigned j = 0u;j < horizontal_split_count + 1u; j++){
			vertices[index] = glm::vec3(j * width_split_step - width * 0.5f,
										0.0f,
...
//	             /* inform OpenGL that the data is modified once, but used often */GL_STATIC_DRAW);
	...
	glVertexAttribPointer(static_cast<unsigned int>(bonobo::shader_bindings::texcoords),
						  ...
						  reinterpret_cast<GLvoid const*>(vertices_size));
                                ...
                                unsigned int const latitude_split_count)
{
	...
	auto const vertices_nb = (longitude_split_count + 1) * (latitude_split_count + 1);
	...
	float theta = 0.0f;
	float phi = 0.0f;
	float theta_angle_step = glm::two_pi<float>() / static_cast<float>(longitude_split_count); //azimuth angle
	float phi_angle_step = glm::pi<float>() / static_cast<float>(latitude_split_count);  //polar angle
	for(unsigned i=0u;i<latitude_split_count+1;i++){ // down -> up
		...
		for(unsigned j=0u;j<longitude_split_count+1;j++){ //right ->left
			...
			if (phi == 0.0f || phi == glm::pi<float>()) {
				...
				tangents[index] = glm::normalize(glm::vec3(radius * std::cos(theta) * std::sin(phi),
												  0.0f,
											-radius * std::sin(theta) * std::sin(phi)));
				binormals[index] = glm::normalize(glm::vec3(radius * std::sin(theta) * std::cos(phi),
											 radius * std::sin(phi),
	...
	// create index array
	index = 0u;
	auto index_sets = std::vector<glm::uvec3>(latitude_split_count * longitude_split_count * 2);
                               ...
                               unsigned int const minor_split_count)
{
	...
	size_t index = 0u;
	float const major_step = glm::two_pi<float>() / static_cast<float>(major_split_count);
	float const minor_step = glm::two_pi<float>() / static_cast<float>(minor_split_count);
	...
	for (unsigned int i = 0u; i <= major_split_count; ++i) {
		...
		for (unsigned int j = 0u; j <= minor_split_count; ++j, ++index) {
			...
			// 法线方向（指向圆管外侧）
			normals[index] = glm::normalize(glm::vec3(
				cos_u * cos_v,
				sin_u * cos_v,
				sin_v
			));
			...
			// 副切线方向
			binormals[index] = glm::normalize(glm::cross(normals[index], tangents[index]));

			// 纹理坐标
			texcoords[index] = glm::vec2(
				static_cast<float>(i) / major_split_count,
	...
	auto index_sets = std::vector<glm::uvec3>(major_split_count * minor_split_count * 2);
		index = 0u;
