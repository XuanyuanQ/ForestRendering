...
static std::array<char const *, 3> const cull_mode_labels{"Disabled", "Back faces", "Front faces"};
static std::array<char const *, 3> const polygon_mode_labels{"Fill", "Line", "Point"};
} // namespace local
...
static std::vector<std::uint8_t> getTextureData(std::string const &filename, std::uint32_t &width,
												std::uint32_t &height, bool flip) {
	auto const channels_nb = 4u;
	stbi_set_flip_vertically_on_load_thread(flip ? 1 : 0);
	unsigned char *image_data = stbi_load(filename.c_str(), reinterpret_cast<int *>(&width),
...
std::vector<bonobo::mesh_data> bonobo::loadObjects(std::string const &filename,
	...
	for (size_t j = 0; j < assimp_scene->mNumMeshes; ++j) {
		...
		if (material_id >= assimp_scene->mNumMaterials)
			LogError("Mesh \"%s\" has a material index of %u, but only %u materials are present.",
					 assimp_object_mesh->mName.C_Str(), material_id, assimp_scene->mNumMaterials);
		else
	...
	for (size_t i = 0; i < assimp_scene->mNumMaterials; ++i) {
		...
		auto const process_texture = [&bindings, &material, i, &parent_folder, &texture_count](
										 aiTextureType type, std::string const &type_as_str,
										 std::string const &name) {
			if (material->GetTextureCount(type)) {
				auto const texture_start_time = std::chrono::high_resolution_clock::now();

				if (material->GetTextureCount(type) > 1)
					LogWarning("Material \"%s\" has more than one %s texture: discarding all but "
							   "the first one.",
				...
				utils::opengl::debug::nameObject(
					GL_TEXTURE, id, std::string(material->GetName().C_Str()) + " " + type_as_str);

				auto const texture_end_time = std::chrono::high_resolution_clock::now();
				LogTrivia(
					"│ %s Texture \"%s\" loaded in %.3f ms", bindings.size() == 1 ? "┌" : "├",
					path.C_Str(),
					std::chrono::duration<float, std::milli>(texture_end_time - texture_start_time)
						.count());
			}
		};
		...
		LogTrivia("│ %s Material \"%s\" loaded in %.3f ms", bindings.empty() ? "╺" : "┕",
				  material->GetName().C_Str(),
				  std::chrono::duration<float, std::milli>(material_end_time - material_start_time)
					  .count());
	}
	...
	LogInfo("assimp_scene mNumMeshes: %u ", assimp_scene->mNumMeshes);
	for (size_t j = 0; j < assimp_scene->mNumMeshes; ++j) {
		LogInfo("1mNumMeshes j: %u ", j);
		...
		if ((assimp_object_mesh->mPrimitiveTypes &
			 ~static_cast<uint32_t>(aiPrimitiveType_POINT | aiPrimitiveType_NGONEncodingFlag)) !=
				0u &&
			(assimp_object_mesh->mPrimitiveTypes &
			 ~static_cast<uint32_t>(aiPrimitiveType_LINE | aiPrimitiveType_NGONEncodingFlag)) !=
				0u &&
			(assimp_object_mesh->mPrimitiveTypes &
			 ~static_cast<uint32_t>(aiPrimitiveType_TRIANGLE | aiPrimitiveType_NGONEncodingFlag)) !=
				0u) {
			LogError("Unsupported mesh \"%s\": uses multiple primitive types",
					 assimp_object_mesh->mName.C_Str());
			continue;
		}
		LogInfo("2mNumMeshes j: %u ", j);
		if ((assimp_object_mesh->mPrimitiveTypes &
		...
		auto const vertices_size =
			static_cast<GLsizeiptr>(assimp_object_mesh->mNumVertices * sizeof(glm::vec3));
		...
		object.vertices.reserve(assimp_object_mesh->mNumVertices);

		for (unsigned int i = 0; i < assimp_object_mesh->mNumVertices; ++i) {
			auto const &v = assimp_object_mesh->mVertices[i];
			object.vertices.emplace_back(v.x, v.y, v.z);
		}
		...
		if (assimp_object_mesh->HasNormals()) {
			...
			glVertexAttribPointer(static_cast<unsigned int>(bonobo::shader_bindings::normals), 3,
								  GL_FLOAT, GL_FALSE, 0,
		...
		if (assimp_object_mesh->HasTextureCoords(0u)) {
			...
			glVertexAttribPointer(static_cast<unsigned int>(bonobo::shader_bindings::texcoords), 3,
								  GL_FLOAT, GL_FALSE, 0,
		...
		if (assimp_object_mesh->HasTangentsAndBitangents()) {
			...
			glVertexAttribPointer(static_cast<unsigned int>(bonobo::shader_bindings::tangents), 3,
								  GL_FLOAT, GL_FALSE, 0,
			...
			glVertexAttribPointer(static_cast<unsigned int>(bonobo::shader_bindings::binormals), 3,
								  GL_FLOAT, GL_FALSE, 0,
		...
		LogInfo("35mNumMeshes j: %u ", j);
		objects.push_back(object);
		LogInfo("45mNumMeshes j: %u ", j);
		auto const mesh_end_time = std::chrono::high_resolution_clock::now();
		LogInfo("6mNumMeshes j: %u ", j);
	...
	LogInfo("┕ Scene loaded in %.3f s: %u textures loaded in %.3f s and %zu meshes in %.3f s",
			std::chrono::duration<float>(scene_end_time - scene_start_time).count(), texture_count,
...
GLuint bonobo::createTexture(uint32_t width, uint32_t height, GLenum target, GLint internal_format,
	...
	switch (target) {
	case GL_TEXTURE_1D:
		glTexImage1D(target, 0, internal_format, static_cast<GLsizei>(width), 0, format, type,
					 data);
		break;
	case GL_TEXTURE_2D:
...
GLuint bonobo::loadTexture2D(std::string const &filename, bool generate_mipmap) {
	...
	GLuint texture =
		bonobo::createTexture(width, height, GL_TEXTURE_2D, GL_RGBA, GL_RGBA, GL_UNSIGNED_BYTE,
...
GLuint bonobo::generateCubemap(std::string const &path, GLenum target) {
	std::uint32_t width, height;
	...
	glTexImage2D(target, 0, GL_RGBA, static_cast<GLsizei>(width), static_cast<GLsizei>(height), 0,
...
GLuint bonobo::loadTextureCubeMap(std::string const &posx, std::string const &negx,
	...
	{
		generateCubemap(posx, GL_TEXTURE_CUBE_MAP_POSITIVE_X);
		generateCubemap(negy, GL_TEXTURE_CUBE_MAP_NEGATIVE_Y);
		generateCubemap(posy, GL_TEXTURE_CUBE_MAP_POSITIVE_Y);
		generateCubemap(negz, GL_TEXTURE_CUBE_MAP_NEGATIVE_Z);
		generateCubemap(posz, GL_TEXTURE_CUBE_MAP_POSITIVE_Z);
	}
...
GLuint bonobo::createProgram(std::string const &vert_shader_source_path,
	...
	GLuint vertex_shader =
		utils::opengl::shader::generate_shader(GL_VERTEX_SHADER, vertex_shader_source);
	if (vertex_shader == 0u)
		return 0u;
	...
	GLuint fragment_shader =
		utils::opengl::shader::generate_shader(GL_FRAGMENT_SHADER, fragment_shader_source);
...
void bonobo::displayTexture(glm::vec2 const &lower_left, glm::vec2 const &upper_right,
							GLuint texture, GLuint sampler, glm::ivec4 const &swizzle,
							glm::ivec2 const &window_size, bool linearise, float nearPlane,
							float farPlane) {
...
GLuint bonobo::createFBO(std::vector<GLuint> const &color_attachments, GLuint depth_attachment) {
...
void bonobo::renderBasis(float thickness_scale, float length_scale,
						 glm::mat4 const &view_projection, glm::mat4 const &world) {
	if (basis.shader == 0u)
		return;
...
bool bonobo::uiSelectCullMode(std::string const &label, enum cull_mode_t &cull_mode) noexcept {
	...
	bool was_modified =
		ImGui::Combo(label.c_str(), &cull_mode_index, local::cull_mode_labels.data(),
...
bool bonobo::uiSelectPolygonMode(std::string const &label,
	...
	bool was_modified =
		ImGui::Combo(label.c_str(), &polygon_mode_index, local::polygon_mode_labels.data(),
...
void setupBasisData() {
	...
	assert(basis.ibo != 0);
	std::array<glm::uvec3, 16> const indices = {// Body: Left
...
void createDebugTexture() {
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, debug_texture_width, debug_texture_height, 0, GL_RGBA,
				 GL_UNSIGNED_BYTE, debug_texture_content.data());
	glBindTexture(GL_TEXTURE_2D, 0u);
