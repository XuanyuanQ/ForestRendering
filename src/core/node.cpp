...
void Node::render(glm::mat4 const &view_projection, glm::mat4 const &parent_transform,
				  int amount) const {
	if (_program != nullptr) {
		// std::cout << "mR1:" << _transform.GetMatrix() << std::endl;
		render(view_projection, parent_transform * _transform.GetMatrix(), *_program, _set_uniforms,
			   amount);
	}
}

void Node::render(glm::mat4 const &view_projection, glm::mat4 const &world, GLuint program,
				  std::function<void(GLuint)> const &set_uniforms, int amount) const {
	...
	glUniformMatrix4fv(glGetUniformLocation(program, "vertex_model_to_world"), 1, GL_FALSE,
					   glm::value_ptr(world));
	glUniformMatrix4fv(glGetUniformLocation(program, "normal_model_to_world"), 1, GL_FALSE,
					   glm::value_ptr(normal_model_to_world));
	glUniformMatrix4fv(glGetUniformLocation(program, "vertex_world_to_clip"), 1, GL_FALSE,
					   glm::value_ptr(view_projection));

	for (size_t i = 0u; i < _textures.size(); ++i) {

		auto const &texture = _textures[i];
		// printf("textue name:%s\n", std::get<0>(texture).c_str());
	...
	if (_has_indices) {
		if (amount != 0) {
			glDrawElementsInstanced(_drawing_mode, _indices_nb, GL_UNSIGNED_INT, 0, amount);
		} else {
...
void Node::set_name(std::string const &name) { _name = std::string("Render ") + name; }
...
void Node::add_texture(std::string const &name, GLuint tex_id, GLenum type) {
	...
	if (_textures.size() >= max_active_texture_count) {
		LogWarning("Trying to add more textures to an object than supported (%llu); the texture %s "
				   "with ID %u will **not** be added. If you really need that many textures, do "
				   "not use the `Node` class and roll your own solution instead.",
				   max_active_texture_count, name.c_str(), tex_id);
		return;
	}
	if (tex_id == 0u) {
		LogWarning(
			"0 is not a valid texture ID; the texture %s (with ID %u) will **not** be added.",
			name.c_str(), tex_id);
		return;
	}
